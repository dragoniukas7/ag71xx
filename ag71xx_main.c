...
#define AG71XX_DEFAULT_MSG_ENABLE \
	(NETIF_MSG_DRV | NETIF_MSG_PROBE | NETIF_MSG_LINK | NETIF_MSG_TIMER | NETIF_MSG_IFDOWN | NETIF_MSG_IFUP | NETIF_MSG_RX_ERR | NETIF_MSG_TX_ERR)

static int ag71xx_msg_level = 16;
...
static void ag71xx_hw_set_macaddr(struct ag71xx *ag, unsigned char *mac)
{
	...
	t = (((u32)mac[5]) << 24) | (((u32)mac[4]) << 16) | (((u32)mac[3]) << 8) | ((u32)mac[2]);
...
static void ag71xx_sgmii_serdes_init_qca956x(struct device_node *np)
{
	...
	//			 QCA956X_PLL_ETH_SGMII_SERDES_LOCK_DETECT | QCA956X_PLL_ETH_SGMII_SERDES_EN_PLL);
	...
	while (!(__raw_readl(gmac_base + QCA956X_GMAC_REG_SGMII_SERDES) & QCA956X_SGMII_SERDES_LOCK_DETECT_STATUS))
		;
									  ...
									  struct ethtool_cmd *cmd)
{
	...
	return phy_ethtool_gset(phydev, cmd); // phy_ethtool_ksettings_get(phydev, cmd);
}
...
{
	if(likely(port_num < ag71xx_slave_devs_count()))
